    ...
    def _auth(self, url):
        try:
            ...
            if r.status_code == 200:
                ...
                if content.startswith('526172') or content.startswith('504b03') or content.startswith('377abc') or content.startswith('1f8b'):
...
def dispatcher(url_file=None, url=None, max_thread=1, dic=None):
    ...
    for u in urllist:
        ...
        for i in range(1, wwwlen):
            wwwhost += www1[i]
        for ii in range(1,wwwlen,2):
            wwwhosts += www1[ii]

        current_info_dic = deepcopy(dic)  # 深拷贝
        current_info_dic.extend([u + '.rar', u + '.zip', u + '.7z', u + '.gz', u + '.tar', u + '.tgz'])
        current_info_dic.extend([u.replace('.', '') + '.rar', u.replace('.', '') + '.zip', u.replace('.', '') + '.7z', u.replace('.', '') + '.gz', u.replace('.', '') + '.tar', u.replace('.', '') + '.tgz'])
        current_info_dic.extend([u.replace('.', '_') + '.rar', u.replace('.', '_') + '.zip', u.replace('.', '_') + '.7z', u.replace('.', '_') + '.gz', u.replace('.', '_') + '.tar', u.replace('.', '_') + '.tgz'])
        current_info_dic.extend([u.replace('.', '-') + '.rar', u.replace('.', '-') + '.zip', u.replace('.', '-') + '.7z', u.replace('.', '-') + '.gz', u.replace('.', '-') + '.tar', u.replace('.', '-') + '.tgz'])
        current_info_dic.extend([wwwhost + '.rar', wwwhost + '.zip', wwwhost + '.7z', wwwhost + '.gz', wwwhost + '.tar', wwwhost + '.tgz'])
        current_info_dic.extend([u.split('.', 1)[-1] + '.rar', u.split('.', 1)[-1] + '.zip', u.split('.', 1)[-1] + '.7z', u.split('.', 1)[-1] + '.gz', u.split('.', 1)[-1] + '.tar', u.split('.', 1)[-1] + '.tgz'])
        current_info_dic.extend([wwwhosts + '.rar', wwwhosts + '.zip', wwwhosts + '.7z', wwwhosts + '.gz', wwwhosts + '.tar', wwwhosts + '.tgz'])
...
if __name__ == '__main__':
    ...
    info_dic = ['bak.rar', 'bak.zip', 'bak.7z', 'bak.gz', 'bak.tgz', 'backup.rar', 'backup.zip', 'backup.7z', 'backup.gz', 'backup.tgz', 'www.zip', 'www.rar', 'www.7z', 'www.gz', 'www.tgz', 'web.rar', 'web.zip', 'web.7z', 'web.gz', 'web.tgz', 'wwwroot.rar', 'wwwroot.zip', 'wwwroot.7z', 'wwwroot.gz', 'wwwroot.tgz', ]
